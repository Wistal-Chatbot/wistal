/**
 * The prompt registry: every editable prompt key, plus the compiled-in default
 * text for each.
 *
 * These defaults serve three purposes:
 *  1. **Seed** — the initial migration inserts them as version 1.
 *  2. **Fallback** — if the DB read fails or a key has no rows yet, the chat
 *     still works off these instead of erroring (see `prompt-store.ts`).
 *  3. **Reference** — the admin UI shows them so an edit can be compared
 *     against, or reset to, the shipped wording.
 *
 * Deliberately free of `server-only` and `db` imports so the admin client can
...
const CHAT_WEB_SEARCH_DEFAULT = `# Wyszukiwanie w internecie
...
const CHAT_BIZRAPORT_DEFAULT = `# Dane o firmach (BizRaport)
...
const CHAT_GOOGLE_RATING_DEFAULT = `# Ocena firmy w Google (Google Places)
...
const DATA_ANSWER_DEFAULT = `Jesteś asystentem ERP firmy Wistal (handel wyrobami hutniczymi/stalowymi).
...
export const PROMPT_DEFAULTS: Record<PromptKey, string> = {
  chat_system: CHAT_SYSTEM_DEFAULT,
  chat_web_search: CHAT_WEB_SEARCH_DEFAULT,
  chat_bizraport: CHAT_BIZRAPORT_DEFAULT,
  chat_google_rating: CHAT_GOOGLE_RATING_DEFAULT,
  data_answer: DATA_ANSWER_DEFAULT,
};

export interface PromptMeta {
  /** Polish label shown in the admin UI. */
  label: string;
  /** When this prompt is sent, so an admin knows what an edit affects. */
  description: string;
  /** Whether `{{ERP_SCHEMA}}` is expanded for this prompt. */
...
export const PROMPT_META: Record<PromptKey, PromptMeta> = {
  chat_system: {
    ...
    description:
      "Wysyłany przy każdej wiadomości w czacie. Zawiera zasady bezpieczeństwa i schemat bazy ERP.",
    supportsSchemaPlaceholder: true,
  },
  chat_web_search: {
    label: "Dodatek: wyszukiwanie w internecie",
    description:
      "Dołączany tylko w rozmowach z włączonym wyszukiwaniem w internecie.",
    supportsSchemaPlaceholder: false,
  },
  chat_bizraport: {
    label: "Dodatek: BizRaport",
    description:
      "Dołączany, gdy skonfigurowano integrację BizRaport (dane o firmach).",
    supportsSchemaPlaceholder: false,
  },
  chat_google_rating: {
    label: "Dodatek: oceny Google",
    description:
      "Dołączany, gdy skonfigurowano integrację Google Places (oceny firm).",
    supportsSchemaPlaceholder: false,
  },
  data_answer: {
    label: "Prompt szybkich akcji (rekord z tabeli)",
    description:
      "Używany przez szybkie akcje typu „rekord z tabeli” — model dostaje gotowe dane, bez SQL i narzędzi.",
    supportsSchemaPlaceholder: false,
  },
};
